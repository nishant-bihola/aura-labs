      ...
      <div className="card-body grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {CATEGORY_KEYS.map((cat) => {
          ...
          return (
            <div
              key={cat}
              className="flex flex-col items-center gap-2.5 p-3 rounded-xl bg-slate-800/40 hover:bg-slate-800/70 transition-colors"
            >
              <div className="relative flex-shrink-0">
                <Ring pct={pct} color={color} />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xl" style={{ transform: 'none' }}>{icon}</span>
                </div>
              </div>
              <div className="text-center w-full">
                ...
                {alloc > 0 && (
                  ...
                  >
                    {isOver
                      ? `$${(spent - alloc).toFixed(0)} over`
                      : `$${(alloc - spent).toFixed(0)} left`}
                  </p>
                )}
              </div>
            </div>
